...
We start by finding obvious spatial outlier.
```{r}
...
myEnv <- raster::stack(system.file('extdata/envs.tif',
                                 package = 'occOutliers'))
...
presOut <- findOutlyingPoints(pres = myPresDF,
                              spOutliers = TRUE,
                              pval = 1e-5)
worldShp <- readRDS(system.file('extdata/worldShpMollwide.rds',
                                package = 'occOutliers'))
plotOutliers(presOut,shpToPlot = worldShp)
```
